...
{	
	...
	public:
		...
		void setStream(const std::stringstream& sstream)
		{
		...
		void next_token()
		{
			...
			{
				throw  "\033[31mUnknown syntax token " +  temp + "\n\033[0m";
			}
...
class SymbolsNode : public Node 
{
	...
	public:
		SymbolsNode(TYPE type):Node(type)
		{
...
{
	public:
		PrimaryNode(TYPE type):Node(type)
		{
...
class WriteNode : public PrimaryNode
{
		...
		WriteNode(TYPE type):PrimaryNode(type)
		{
...
{	
	...
	public:
		...
		FlowControlNode(TYPE type):Node(type)
		{
...
{	
	...
	public:
		...
		StatementNode(TYPE type):Node(type)
		{
...
{	
	...
	public:
		...
		IfNode(TYPE type):StatementNode(type)
		{
...
protected:
	...
	public:
		IfElseNode(TYPE type):StatementNode(type)
		{
...
class IfNotNode : public StatementNode 
{
	public:
		...
		IfNotNode(TYPE type):StatementNode(type)
		{
...
class LoopNode : public StatementNode
{
	...
	public:
		...
		LoopNode(TYPE type):StatementNode(type)
		{
...
{
	public:
		WhileNode(TYPE type):LoopNode(type)
		{
...
{
	public:
		DoWhileNode(TYPE type):LoopNode(type)
		{
...
{
	public:
		RepeatUntilNode(TYPE type):LoopNode(type)
		{
...
class BlockNode : public StatementNode 
{
	public:
		...
		BlockNode(TYPE type):StatementNode(type)
		{
...
class MainNode : public StatementNode 
{
	public:
		...
		MainNode(TYPE type):StatementNode(type)
		{
...
class Parser
{
	...
	public:
		...
		Lexer::KEYWORDS offset_token(int offset)
		{
		...
		std::string offset_token_value(int offset)
		{
			if ( (this->token_count + offset < (int)this->tokens.size())  && (this->token_count + offset >= 0) )
			{
		...
		MainNode* parse()
		{
			...
			{
				throw "\033[31mInvalid syntax " + this->offset_token_value(0) + "\n\033[0m";
			}
			return node;
		}
		...
		MainNode* parse_main_statement()
		{
			...
			{
				throw "\033[31mExpected main statement\n\033[0m";
			}
			...
			{
				throw "\033[31mMain statement should only contain block statement.\n\033[0m";
			}
		...
		Node* parse_statement(Node* root)
		{	
			Node* node0 = nullptr;
			Node* node1 = nullptr;
			...
			if (this->offset_token(0)  == Lexer::KEYWORDS::DO)
			{
				...
				if (this->offset_token(0)  != Lexer::KEYWORDS::WHILE)
				{
					throw "\033[31mIncorrect Do/While statement\n\033[0m";
				}
				...
				{
					throw  "\033[31mExpected semicolon	 \n\033[0m";
				}		
			...
			if (this->offset_token(0)  == Lexer::KEYWORDS::REPEAT)
			{
				...
				{
					throw "\033[31mIncorrect Repeat/Until statement\n\033[0m";
				}
			...
			if (this->offset_token(0)  == Lexer::KEYWORDS::L_BR)
			{
				...
				{
					throw "\033[31mEmpty block statement\n\033[0m";
				}
			...
			if (this->offset_token(0) == Lexer::KEYWORDS::WRITE)
			{
				...
				{
					throw "\033[31mExpected one symbol in write statement\n\033[0m";
				}
			...
			if (this->offset_token(0) == Lexer::KEYWORDS::CONTINUE)
			{
				...
				if (!	in_owner_loop(node))
				{
					throw "\033[31mFlow control statement continue should be within a owner_loop\n\033[0m";
				}
			...
			if (this->offset_token(0) == Lexer::KEYWORDS::SEMICOLON)
			{
				throw "\033[31mSemicolon shoul follow by an primary statement \n\033[0m";				
			}
			else
			if (this->offset_token(0) == Lexer::KEYWORDS::R_BR)
			{
				throw  "\033[31mEmpty single statement \n\033[0m";
			}
			else
			if (this->offset_token(0) == Lexer::KEYWORDS::ELSE)
			{
				throw "\033[31mElse statement should follow by an if statement\n\033[0m";
			}
			else
			if (this->offset_token(0) == Lexer::KEYWORDS::END)
			{
				throw "\033[31mExpected statement\n\033[0m";
			}
			else
			if (this->offset_token(0) == Lexer::KEYWORDS::MAIN)
			{
				throw  "\033[31mMain statement should occur only once \n\033[0m";
			}
			else
			{						
				throw "\033[31mUnknow statement " + this->offset_token_value(0) + "\n\033[0m";
			}			
		...
		{			
			...
			{
				throw "\033[31mExpected left parenrhesis\n\033[0m";
			}
			...
			while (true)
			{
				...
				if (this->offset_token(0)  == Lexer::KEYWORDS::R_PAR)
				{
					...
					{
						throw "\033[31mEmpty parenthesis statement\n\033[0m";
					}
					break;
				}// If there will be errors
				
				if ((this->offset_token(0)  != Lexer::KEYWORDS::SYMBOL) && ( ! first_was_symbol))
				{
					throw"\033[31mExpected symbol before comma\n\033[0m";	
				}
				...
				if (this->offset_token(0)  == Lexer::KEYWORDS::SYMBOL)
				{
					...
					if (std::find(node.symbols.begin(), node.symbols.end(), this->offset_token_value(0)[0]) != node.symbols.end())
					{
						...
						throw "\033[31mSymbol " + this->offset_token_value(0) + " is already given \n\033[0m";
					}
					...
					if (!isGlobal)
					{
						...
						{
							throw "\033[31mThe symbol " + this->offset_token_value(0) + " is not given in main symbols\n\033[0m";
						}
					}
				...
				if (las_was_comma)
				{
					throw "\033[31mExpected symbol after comma \n\033[0m";
				}
...
void print(Node* node, int shift = 0)
{
...
class Compiler
{
	...
	public:
		...
		void compile_node(Node* node, std::vector<std::string>& lines, size_t& state_number, const std::string& globalSymbols)
		{
			std::string temp = "";
			int curent_line;
			...
			switch (node->type)
			{
				...
				default:
						...
						throw "\033[31mUnexpected node to compile.\n\033[0m";	
		...
		void put_flow_controls(Node* node, std::vector<std::string>& lines, size_t& state_number, const std::string& globalSymbols)
		{
			std::string temp;
